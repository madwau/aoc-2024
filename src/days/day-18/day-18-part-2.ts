...
function isReachable(nanoseconds: number): boolean {
  ...
  const cost = dijkstra(neighbors, { x: 0, y: 0 }, pos =>
    equals(pos, { x: MAX_COORD, y: MAX_COORD }),
  );

  return cost !== Infinity;
}

function firstByteBlockingExit(): Byte {
  let low = 1;
...
console.log(Object.values(firstByteBlockingExit()).join(','));
